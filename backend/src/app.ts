...
export async function initialize() {
  try {
    ...
    await waitUntilReady(checkSupertokensReadiness);
    console.log("  Verifying postgrest is up...");
    await waitUntilReady(checkPostgrestReadiness);
...
async function waitUntilReady(func:any, interval = 5000, maxAttempts = 10) {
  let attempt = 1;
  while (attempt <= maxAttempts) {
    try {
      await func(); // Call the provided function
      return; // If successful, exit the loop
    } catch (error:any) {
      console.log(`Attempt ${attempt}: ${error.message}`);
    }
    
    // Wait for a delay before making the next attempt
    await new Promise(resolve => setTimeout(resolve, interval));
    attempt++;
  }

  throw new Error(`Function did not respond successfully after ${maxAttempts} attempts.`);
}
